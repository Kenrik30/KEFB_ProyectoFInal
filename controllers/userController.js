...
exports.createUser = async (req, res) => {
    try {
        const { nombre, apellido, email, nUsuario, password,rol,status } = req.body;
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(password, salt);

        const newUser = new User({ nombre, apellido, email, nUsuario,rol, status, password: hashedPassword });
        await newUser.save();
